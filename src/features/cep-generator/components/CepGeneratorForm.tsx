...
import { generateCep, formatCep, validateCep } from "@/features/cep-generator/utils/generate";

export function CepGeneratorForm() {
  const t = useTranslations("cep-generator");
  ...
  const handleValidate = useCallback(() => {
    ...
    if (cleaned.length === 8) {
      setValidationResult(validateCep(cep) ? "valid" : "invalid");
    } else {
  ...
  const handleBatchGenerate = useCallback(() => {
    ...
    const ceps = Array.from({ length: count }, () => generateCep());
    setBatchResult(ceps.join("\n"));
    ...
    <section className="mx-auto flex w-full max-w-3xl flex-col gap-10 px-4 py-12 sm:px-6 lg:px-8">
      ...
      {mode === "single" && (
        <div className="flex flex-col gap-4">
          {/* Input */}
          <div>
            <label htmlFor="cep-input" className="mb-2 block text-sm font-medium text-foreground">
              {t("resultLabel")}
            </label>
            <input
              id="cep-input"
      ...
      {mode === "batch" && (
        <div className="flex flex-col gap-4">
          {/* Quantity input */}
          <div>
            <label htmlFor="cep-batch-count" className="mb-2 block text-sm font-medium text-foreground">
              {t("batchCount")}
            </label>
            <input
              id="cep-batch-count"
          ...
          {batchResult && (
            <>
              <div>
                <label htmlFor="cep-batch-result" className="mb-2 block text-sm font-medium text-foreground">
                  {t("batchResultLabel")}
                </label>
                <textarea
                  id="cep-batch-result"
