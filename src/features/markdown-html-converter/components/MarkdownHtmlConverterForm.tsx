...
import { markdownToHtml, htmlToMarkdown } from "@/features/markdown-html-converter/utils/convert";

export function MarkdownHtmlConverterForm() {
  const t = useTranslations("markdown-html-converter");
  const [activeTab, setActiveTab] = useState<"md" | "html">("md");
    ...
    <section className="mx-auto flex w-full max-w-3xl flex-col gap-10 px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex gap-1 rounded-md border border-border bg-muted p-1">
          <button
            type="button"
            onClick={() => { setActiveTab("md"); setResult(""); setError(false); }}
            className={`flex-1 rounded-md px-4 py-2 text-sm font-medium transition-colors ${
              activeTab === "md" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
            }`}
          >
            {t("mdToHtmlTab")}
          </button>
          <button
            type="button"
            onClick={() => { setActiveTab("html"); setResult(""); setError(false); }}
            className={`flex-1 rounded-md px-4 py-2 text-sm font-medium transition-colors ${
        ...
        <div>
          <label htmlFor="mhc-input" className="mb-2 block text-sm font-medium text-foreground">
            {activeTab === "md" ? "Markdown" : "HTML"}
          </label>
          <textarea
            id="mhc-input"
            value={input}
        ...
        {result && (
          <div>
            <label htmlFor="mhc-result" className="mb-2 block text-sm font-medium text-foreground">{t("outputLabel")}</label>
            <textarea
              id="mhc-result"
              readOnly
              value={result}
