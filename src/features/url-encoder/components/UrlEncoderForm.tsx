...
import { encodeUrl, decodeUrl, encodeUrlFull, decodeUrlFull } from "@/features/url-encoder/utils/encode";

export function UrlEncoderForm() {
  const t = useTranslations("url-encoder");
  ...
  const handleEncode = useCallback(() => {
    try {
      setResult(mode === "full" ? encodeUrlFull(input) : encodeUrl(input));
      setError(false);
      setCopied(false);
      setCopyError(false);
    } catch {
  ...
  const handleDecode = useCallback(() => {
    try {
      setResult(mode === "full" ? decodeUrlFull(input) : decodeUrl(input));
      setError(false);
      setCopied(false);
      setCopyError(false);
    } catch {
    ...
    <section className="mx-auto flex w-full max-w-3xl flex-col gap-10 px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center gap-4">
          <span className="text-sm font-medium text-foreground">{t("encodeMode")}</span>
        ...
        {result && (
          <div>
            <label htmlFor="url-result" className="mb-2 block text-sm font-medium text-foreground">
              {t("resultLabel")}
            </label>
            <textarea
              id="url-result"
